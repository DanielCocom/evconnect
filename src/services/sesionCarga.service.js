...
class SesionCargaService {
     ...
     */
    static async startChargeSession(
        userId,
        chargerId,
        chargeType,
        durationMinutes
        ...
        if (!isChargerConnected) {
            throw {
                status: 503,
                message: `El cargador #${chargerId} no está conectado al sistema. No se puede iniciar la sesión.`,
            };
        }

        // Verifica si el usuario tiene una tarjeta predeterminada
        const defaultPaymentMethod =
            await MetodoPagoService.getDefaultPaymentMethod(userId);
        ...
        if (activeSession) {
            throw {
                status: 409,
                message: "Ya tienes una sesión de carga activa o pendiente.",
            };
        }
        ...
        if (cargador.estado !== "disponible") {
            throw {
                status: 409,
                message: `El cargador #${chargerId} no está disponible. Estado: ${cargador.estado}`,
            };
        }
        ...
        if (!tarifa) {
            throw {
                status: 400,
                message: `No hay una tarifa vigente por minuto para este cargador (${cargador.tipo_carga}).`,
            };
        }
        ...
        if (paymentIntent.status !== "requires_capture") {
            ...
            throw {
                status: 402,
                message:
                    "El pago no pudo ser autorizado. Verifique sus fondos o tarjeta.",
            };
        }
        ...
        try {
            ...
            console.log(
                `[SesionCarga] Comando START enviado al cargador ${chargerId}`
            );
        } catch (iotError) {
            // Si el IoT no responde, debemos revertir la transacción y liberar el cargador
            console.error(
                "[SesionCarga] Error al enviar comando START al IoT:",
                iotError
            );
            ...
            throw {
                status: 503,
                message: "Cargador no responde. Sesión cancelada.",
                errors: iotError,
            };
        }
    ...
    static async completeChargeSession(sessionId, userId) {
        ...
        const tiempoTranscurridoMs = Math.max(
            0,
            ahora.getTime() - inicioSesion.getTime()
        );
        ...
        try {
            ...
            console.log(
                `[SesionCarga] Comando STOP enviado al cargador ${sesion.id_cargador}`
            );
        } catch (iotError) {
            // Loguear el error, pero el cobro debe continuar
            console.error(
                "[SesionCarga] Error al enviar comando STOP a IoT:",
                iotError
            );
        }
        ...
        let capture;
        try {
            capture = await StripeService.capturePaymentIntent(
                paymentIntentId,
                montoFinalNum
            );
            console.log(
                `[SesionCarga] Pago capturado: $${montoFinalNum} MXN (${tiempoTranscurridoMin} minutos)`
            );
        } catch (stripeError) {
